...
def linear_w_regression(x_data: np.array,y_data: np.array,sigma_x:np.array,sigma_y:np.array):
    """
    Esta función devuelve la pendiente e intercepto de una regresión lineal
    por medio del método de mínimos cuadrados ponderados.

    Args:
        x_data (np.array): Datos de la variable independiente
        y_data (np.array): Datos de la variable dependiente
        sigma_x (np.array): Incertidumbre de cada dato de la variable independiente
        sigma_y (np.array): Incertidumbre de cada dato de la variable dependiente

    Returns:
        np.array: Array de datos con la pendiente, incertidumbre de la pendiente, intercepto y la incertidumbre del
                  intercepto.
    """
    w = (1/sigma_y)**2
...
def linear_regression(x_data: np.array, y_data: np.array):
